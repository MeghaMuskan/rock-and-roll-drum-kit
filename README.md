...
**What I designed:**
- Two screens: a landing page and the drum kit page
- Each drum pad as a reusable `DrumPad` component with hover and pressed states
- A `Smart Animate` prototype transition between screens to validate the interaction feel before implementing it in CSS
- The vintage color palette (deep crimson `#120808`, signal red `#e8350a`, aged cream `#e8d5a0`, gold `#c8a830`) was locked in Figma and then translated directly into CSS custom properties

**Why this mattered:** Designing in Figma first meant the CSS animation durations, the pad sizes, the glow radii, and the layout spacing were all decided visually — not guessed in code. The Figma Smart Animate prototype also served as a spec for how the `pressed` state animation should feel (scale down + opacity + box-shadow in one 80ms burst).
